...
it(`getConnectorFiles[typings|signature]: schema with 'Buffer' and 'string' units`, () => {
  const { typings, signature } = schema.getConnectorFiles({
    id: 'testConnector',
    options: {
      model: {
        type: 'stringUnion',
  ...
  expect(signature.filePath).toBe(
    config.adapter.connectorSignaturePath('testConnector')
  )

  expect(signature.body).toBe(
    dedent(`
      /**
       * The file is generated by SPADAR CLI v. ${config.version}
       * DO NOT EDIT IT MANUALLY because it could be automatically rewritten
